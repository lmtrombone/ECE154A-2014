...
start:      #For each test case
            ...
            addi $t8, $0, 16384                 # binary 0100 0000 0000 0000
            slt  $t9, $t0, $t1                  # is 16384 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 8192                  # binary 0010 0000 0000 0000
            slt  $t9, $t0, $t1                  # is 8192 < exponent?
            bne  $t9, $zero, found1             # if so, move on

            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 4096                  # binary 0001 0000 0000 0000
            slt  $t9, $t0, $t1                  # is 4096 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 2048                  # binary 0000 1000 0000 0000
            slt  $t9, $t0, $t1                  # is 2048 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 1024                  # binary 0000 0100 0000 0000
            slt  $t9, $t0, $t1                  # is 1024 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 512                   # binary 0000 0010 0000 0000
            slt  $t9, $t0, $t1                  # is 512 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 216                   # binary 0000 0001 0000 0000
            slt  $t9, $t0, $t1                  # is 216 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 128                   # binary 0000 0000 1000 0000
            slt  $t9, $t0, $t1                  # is 128 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 64                    # binary 0000 0000 0100 0000
            slt  $t9, $t0, $t1                  # is 64 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 32                    # binary 0000 0000 0010 0000
            slt  $t9, $t0, $t1                  # is 32 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 16                    # binary 0000 0000 0001 0000
            slt  $t9, $t0, $t1                  # is 16 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 8                     # binary 0000 0000 0000 1000
            slt  $t9, $t0, $t1                  # is 8 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 4                     # binary 0000 0000 0000 0100
            slt  $t9, $t0, $t1                  # is 4 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 2                     # binary 0000 0000 0000 0010
            slt  $t9, $t0, $t1                  # is 2 < exponent?
            bne  $t9, $zero, found1             # if so, move on
            
            addi $s4, $s4, -1                   # decrement bit counter
            addi $t8, $0, 1                     # binary 0000 0000 0000 0001
